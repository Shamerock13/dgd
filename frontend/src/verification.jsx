...
export default function VerificationAdmin({api,flash,brands=[],items=[],twins=[]}){
  ...
  const researchTask=async task=>{
    ...
    try{
      const result=await api(`/api/enrichment/tasks/${task.fragrance_id}/research`,{method:'POST'});
      flash(`${result.findings_created||0} Ergänzungen, ${result.twins_created||0} neue Duftzwilling-Hinweise; ${result.known_twins_excluded||0} bekannte Zwillinge ausgeschlossen.`);
      await load();
  ...
  return <div className="verification-admin">
    ...
    <section className="verification-review-block"><div className="verification-section-head"><div><span className="kicker">Datenprüfung</span><h3>Fehlende Duftdaten</h3><p>Wähle einen unvollständigen Duft aus und lasse gezielt nur dessen offene Felder recherchieren.</p></div><button type="button" onClick={refreshGaps} disabled={loading||!!researchingTask}>Datenlücken neu prüfen</button></div><div className="verification-gap-list">{tasks.map(task=><article key={task.id}><div><b>{task.brand_name} – {task.fragrance_name}</b><span>{(task.missing_fields||[]).map(field=>fieldLabels[field]||field).join(' · ')}</span></div><strong>{(task.missing_fields||[]).length} offen</strong><button type="button" className="approve" disabled={!!researchingTask} onClick={()=>researchTask(task)}><Sparkles size={15}/>{researchingTask===task.fragrance_id?'Gemini sucht …':'Mit Gemini ergänzen'}</button></article>)}</div>{!tasks.length&&<div className="verification-empty">Aktuell sind keine offenen Datenlücken erfasst.</div>}</section>
