...
async function navCount(url){try{const response=await fetch(url);if(!response.ok)return 0;const data=await response.json();return Array.isArray(data)?data.length:Number(data?.count||0)}catch{return 0}}
