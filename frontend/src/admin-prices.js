...
function errorMessage(detail, fallback) {
  ...
  if (detail && typeof detail === 'object') return detail.message || detail.msg || JSON.stringify(detail, null, 2);
  return fallback;
}
...
async function request(url, options = {}) {
  ...
  if (!response.ok) {
    ...
    try { const body = await response.json(); message = errorMessage(body.detail ?? body, message); } catch {}
...
function savedOffersHtml() {
  if (!discovery?.fragrance_id) return '';
  if (!priceOverview) return '<section class="price-card"><h3>Gespeicherte Angebote</h3><p class="price-discovery-empty">Noch keine Preisdaten geladen.</p></section>';
  const offers = priceOverview.offers || [];
  if (!offers.length) return '<section class="price-card"><h3>Gespeicherte Angebote</h3><p class="price-discovery-empty">Für diesen Duft ist noch kein Angebot gespeichert.</p></section>';
  const cheapest = priceOverview.cheapest;
  return `<section class="price-card">
    <h3>Gespeicherte Angebote</h3>
    ${cheapest ? `<div class="price-result"><b>Günstigstes Angebot: ${euro.format(cheapest.total_eur)}</b><span>${esc(cheapest.retailer.name)}${cheapest.size_ml ? ` · ${esc(cheapest.size_ml)} ml` : ''}</span></div>` : ''}
    <div class="price-retailer-list">${offers.map(offer => `<article>
      <div><small>${offer.in_stock ? 'Lieferbar' : 'Nicht lieferbar'} · ${offer.size_ml ? `${esc(offer.size_ml)} ml` : 'Größe offen'}</small><b>${esc(offer.product_name || offer.retailer.name)}</b><span>${euro.format(offer.price_eur)} + ${euro.format(offer.shipping_eur)} Versand = ${euro.format(offer.total_eur)}</span><a href="${esc(offer.product_url)}" target="_blank" rel="noreferrer">Zum Händler</a></div>
      <em class="${offer.in_stock ? 'active' : ''}">${offer.price_per_100ml_eur != null ? `${euro.format(offer.price_per_100ml_eur)} / 100 ml` : esc(offer.retailer.name)}</em>
    </article>`).join('')}</div>
  </section>`;
}
...
async function loadOverview(fragranceId) {
  if (!fragranceId) { priceOverview = null; return; }
  priceOverview = await request(`/api/prices/fragrances/${fragranceId}?days=90`);
}

async function load() {
  try {
    [retailers, fragrances] = await Promise.all([request('/api/prices/retailers'), request('/api/fragrances')]);
    if (discovery?.fragrance_id) await loadOverview(discovery.fragrance_id);
    render();
...
async function runDiscovery(event) {
  ...
  try {
    discovery = await request('/api/prices/discovery/search', {method:'POST', body:JSON.stringify({fragrance_id})});
    await loadOverview(fragrance_id);
    render();
...
async function acceptCandidate(event) {
  ...
  if (!fragrance_id) { alert('Der ausgewählte Duft konnte nicht mehr zugeordnet werden. Bitte die Händlersuche erneut starten.'); return; }
  const size = prompt('Größe in ml (optional):', '100');
  if (size === null) return;
  const sizeMl = size.trim() ? Number(size.replace(',', '.')) : null;
  if (sizeMl !== null && (!Number.isFinite(sizeMl) || sizeMl <= 0 || sizeMl > 5000)) { alert('Bitte eine gültige Größe zwischen 0,1 und 5000 ml eingeben.'); return; }
...
async function saveOffer(event) {
  ...
  try {
    ...
    panel.querySelector('.price-result').innerHTML=`<b>Gespeichert</b><span>${esc(result.offer.retailer.name)} · ${euro.format(result.offer.total_eur)}</span>`;
    if (discovery?.fragrance_id === data.fragrance_id) { await loadOverview(data.fragrance_id); render(); }
