...
export default function MediaUpload({item, onChanged, flash}) {
  ...
  return <section className="media-upload-box">
    <div className="media-upload-copy"><ImagePlus/><div><b>Lokale Medienablage</b><span>{item?.id?'JPEG, PNG oder WebP bis 8 MB. Die Datei wird dauerhaft im Unraid-Medienordner gespeichert.':'Duft zuerst speichern, danach kann ein Bild hochgeladen werden.'}</span></div></div>
    <div className="media-upload-actions">
      ...
      <button type="button" disabled={!item?.id||working} onClick={()=>inputRef.current?.click()}><UploadCloud size={17}/>{working?'Bitte warten …':isLocal?'Bild ersetzen':'Bild hochladen'}</button>
